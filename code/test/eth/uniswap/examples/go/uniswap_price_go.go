...
/*
	参数：代币地址 - 流动池地址 - 节点地址
*/
func GetExchangeAmount(baseAddr, quoteAddr, liquidityAddr, etherNode string) PriceM {
	var priceM PriceM
	client, err := ethclient.Dial("https://mainnet.infura.io/v3/d6624913c27d4f5a9540f071767b4d49")
	if err != nil {
		panic(err)
	}
	if etherNode != "" {
		client, err = ethclient.Dial(etherNode)
		if err != nil {
			panic(err)
		}
	}

	baseAddress := common.HexToAddress(baseAddr)
	quoteAddress := common.HexToAddress(quoteAddr)

	pairAddress := common.HexToAddress(liquidityAddr)

	baseInstance, err := NewPairContract(baseAddress, client)
	if err != nil {
		panic(err)
	}

	quoteInstance, err := NewPairContract(quoteAddress, client)
	if err != nil {
		panic(err)
	}

	pairInstance, err := NewPairContract(pairAddress, client)
	if err != nil {
		panic(err)
	}

	baseBalance, err := baseInstance.BalanceOf(&bind.CallOpts{}, pairAddress)
	if err != nil {
		panic(err)
	}

	quoteBalance, err := quoteInstance.BalanceOf(&bind.CallOpts{}, pairAddress)
	if err != nil {
		panic(err)
	}
	...
	priceM.Reserve0 = reserves.Reserve0
	priceM.Reserve1 = reserves.Reserve1
	priceM.BlockTimestampLast = reserves.BlockTimestampLast
	...
	baseAmountInPool := new(big.Float).SetInt(baseBalance)
	quoteAmountInPool := new(big.Float).SetInt(quoteBalance)

	fmt.Printf("\n baseAmountInPool : %+v\n", baseAmountInPool)
	fmt.Printf("\n quoteAmountInPool : %+v\n", quoteAmountInPool)

	price := new(big.Float).Quo(quoteAmountInPool, baseAmountInPool)
	fmt.Println(price)
	priceM.Price = price
	return priceM
}
...
// PairContractMetaData contains all meta data concerning the PairContract contract.
var PairContractMetaData = &bind.MetaData{
...
// Deprecated: Use PairContractMetaData.ABI instead.
var PairContractABI = PairContractMetaData.ABI
