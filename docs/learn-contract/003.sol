...
contract Ballot {
    ...
    function delegate(address to) external {
        // 指定引用
        Voter storage sender = voters[msg.sender];                  // 这里取到的 sender 是引用类型。
        require(sender.weight != 0, "You have no right to vote");   // 选票不为0
        require(!sender.voted, "You already voted.");               // 还未投票
        ...
        while (voters[to].delegate != address(0)) { // 对应的委托地址不为0地址的话，就一直循环委托向下传递
